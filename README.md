...
4. **Viajes y Trayectos (`Viaje` & `Trayecto`)**: Estructura de relación jerárquica unívoca. *Un viaje* general consta de *N trayectos* ordenados cronológica y geográficamente.
...
### Paso A: Levantar el Servidor API Backend (`nxapi-shop`)

1. **Clonar el Repositorio de la API:**
   ```bash
   git clone https://github.com/CarlosCS06/Gestion_Vehiculos_Backend.git nxapi-shop
   cd nxapi-shop
   ```
...
3. **Variables de Entorno (.env):**
   Crea un archivo `.env` en la carpeta raíz del backend con los siguientes parámetros requeridos:
   ```env
   # Cadena de conexión PostgreSQL
   DATABASE_URL="postgresql://usuario:contraseña@localhost:5432/flotagest?schema=public"

   # Secret Key para la firma de tokens JWT
...
4. **Preparar la Base de Datos:**
   ...
   # Precargar datos semilla en la BD
   npm run seed
   ```

5. **Arrancar Entorno de Desarrollo Backend:**
   ```bash
   npm run dev
   ```
...
3. **Configuración de Variables de Entorno (.env):**
   Crea un archivo `.env` en la raíz del proyecto para indicarle a Vite dónde realizar las peticiones AJAX:
   ```env
   VITE_API_URL=http://localhost:3000/api
   ```

4. **Arrancar Entorno de Desarrollo Frontend:**
   ```bash
   npm run dev
   ```
...
* **Ausencia de WebSockets:** La sincronización de información del Dashboard de viajes se actualiza en tiempo real mediante técnicas de consulta periódica ligera (*polling* cada 60 segundos). La integración nativa de WebSockets se encuentra actualmente en desarrollo.

### Próximas Líneas de Trabajo (Roadmap)
1. **Geolocalización en Tiempo Real:** Reemplazar los enlaces actuales de mapas estáticos por un mapa dinámico e interactivo (ej. Leaflet / Mapbox) incrustado en el propio panel de viajes que trace visualmente el itinerario en tiempo real.
2. **Exportador Contable Completo:** Permitir a los gestores la descarga agrupada de costes de reparaciones, revisiones y trayectos de combustible en formatos profesionales descargables PDF y hojas de cálculo CSV listas para enviar a gestorías externas.
3. **Rol Técnico / Taller:** Introducir un nuevo rol de usuario intermedio ("Mecánico") enfocado exclusivamente en gestionar la resolución de Averías y el registro de Revisiones e ITV, aislando su acceso de los datos salariales o perfiles personales de la plantilla de conductores.

---

*Desarrollado con dedicación en el marco de Proyectos Finales de Desarrollo de Aplicaciones Web (DAW).*
